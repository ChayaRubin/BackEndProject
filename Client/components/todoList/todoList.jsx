  ...
  useEffect(() => {
    const fetchTodos = async () => {
      ...
      try {
        ...
        const response = await fetch(`http://localhost:3000/todos/${currentUser.id}`);
        if (!response.ok) {
...
const handleAddTodo = async () => {
  ...
  try {
    ...
    const response = await fetch(`http://localhost:3000/todos/addTodo`, {
      ...
      body: JSON.stringify({
        user_id: Number(currentUser.id), // תיקון כאן
        title: newTitle,
        completed: false,
      }),
    });
    ...
    setTodos((prevTodos) => [...prevTodos, { ...addedTodo, title: newTitle, completed: false, user_id: currentUser.id }]);
    setIsAdding(false);
    setNewTitle("");
  } catch (err) {
    setError(err.message);
